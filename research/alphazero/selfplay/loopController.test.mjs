...
test("各周で selfPlay→append→sample→train が1回ずつ呼ばれ、net が無条件advanceする", () => {
  const dir = tmpDir();
  const { deps, calls } = makeMocks();
  const st = runLoop(deps, { dir, rounds: 3, seedNet: "research/models/genAZ001.json", allowRealRun: true, log: () => {} });
  ...
  assert.equal(st.history[0].prevNet, "research/models/genAZ001.json");
...
test("旧 gated manifest（champion フィールド）を読んでも net として復元する（後方互換）", () => {
  ...
  fs.writeFileSync(path.join(dir, "loop_manifest.json"),
    JSON.stringify({ round: 5, champion: "research/models/genAZ_r5.json", acceptStreak: 1, history: [] }));
  const st = loadLoopState(dir, "seed");
  assert.equal(st.net, "research/models/genAZ_r5.json");
