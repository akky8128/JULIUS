 ...
 *                        --oppNet research/models/gen137.json --sdepth 32 --sms 120。両者の実測ms/turnも集計。
 *
 * MCTS は固定 simulations で決定的に探索する（評価の再現性・速度のため。自己対戦の
 * Dirichlet/温度は OFF）。色は g%2 で交換。結果は JSON 1行で stdout に出す（親が集計）。
 *
 * 使い方:
 *   node evalWorker.mjs --mode vsHeuristic --netA research/models/genAZ_r1.json \
...
async function main() {
  ...
  const netAPath = get("--netA", "research/models/genAZ001.json");
  const netBPath = get("--netB", null);
  const oppNetPath = get("--oppNet", "research/models/gen137.json");
