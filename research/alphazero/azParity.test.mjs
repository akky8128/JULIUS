...
import { TURN_CONTEXT_DIM } from "../../core/az/azFeatures.mjs";
...
function pathsFor(tag) {
  return {
    jsonPath: path.join(projectRoot, "research", "models", `${tag}.json`),
    goldenPath: path.join(projectRoot, "research", "models", `${tag}.golden.json`),
  };
}
...
async function loadOrSkip(t, tag) {
  ...
  try {
    ...
    t.skip(`research/models/${tag}.json が存在しないためスキップ`);
    return null;
  }
...
test("policy head なしモデル(gen001)では evaluatePolicy が Error を投げる", async () => {
  const legacy = await loadNetwork(path.join(projectRoot, "research", "models", "gen001.json"));
