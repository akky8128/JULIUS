 ...
 *
 * 実行: node --test research/nnue/nnuePlayer.test.mjs
 *
...
import { createNnueSearchPlayer, createNnueSearchPlayerFromNet } from "../../core/nnue/nnuePlayer.js";
...
test("createNnueSearchPlayer: 合法な turn と lastSearchInfo を返す", async () => {
  ...
  const player = await createNnueSearchPlayer(rng, {
    netPath: "research/models/gen001.json",
    maxDepth: 3,
    timeBudgetMs: 100,
  });
...
test("createNnueSearchPlayer: .bin パスを渡しても .json に正規化してロードできる", async () => {
  ...
  const player = await createNnueSearchPlayer(rng, {
    netPath: "research/models/gen001.bin",
    maxDepth: 2,
    timeBudgetMs: 100,
  });
  const state = buildFixedState();
...
// 親エージェントが `node research/classic/selfplay.mjs --white nnue:research/models/gen001.json --black
...
test("(任意) selfplay疎通は関数分離の都合上スキップ — 理由をログ出力", () => {
  ...
  // 疎通は research/classic/selfplay.mjs を直接 CLI 実行することで確認する方針とする。
  assert.ok(true);
});
