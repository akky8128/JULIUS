 ...
 *
 * round1 のときだけ --seed-net（既定 models/genAZ001.json）を最新ネットとして使い、
 * 以降は manifest.net を使う。--bench-every K 指定時は K周ごとに現行netで benchmark.mjs を実行。
 *
 * 使い方:
 *   node tools/alphazero/selfplay/runRound.mjs \
 *     --dir tools/alphazero/data/loop2 --seed-net models/genAZ001.json \
...
async function main() {
  ...
  const seedNet = arg("--seed-net", "models/genAZ001.json");
  ...
  // 昨夜のgated run産物(models/genAZ_r3..r65)を上書きしないようにする。
  ...
  const newNetJson = path.join("models", `${netTag}.json`);
  ...
  await pMapLimit(shardGames.map((g, i) => () => run("node", [
    "tools/alphazero/selfplay/selfPlayWorker.mjs",
  ...
  const trainArgs = [
    "tools/nnue/train.py", "--data", batchPath, "--out-dir", "models", "--gen-tag", netTag,
  ...
  if (benchEvery > 0 && round % benchEvery === 0) {
    ...
    await run("node", [
      "tools/alphazero/selfplay/benchmark.mjs",
