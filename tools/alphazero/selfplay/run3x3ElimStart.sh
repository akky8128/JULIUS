...
run_one () {
  ...
  local DIR="tools/alphazero/data/3x3elim_${EXP}"
  local PREFIX="az3e_${EXP}_r"
  echo "=== 3x3 elim-from-start ${EXP} | white=${MW} black=${MB} | sep-nodes | rounds=${ROUNDS} ==="
  rm -rf "$DIR"; rm -f models/${PREFIX}*.json models/${PREFIX}*.bin models/${PREFIX}*.golden.json 2>/dev/null || true
  ...
  UL_BOARD_SIZE=3 UL_SEP_ELIM=1 UL_ELIM_UNLOCK=always UL_MAX_WHITE=$MW UL_MAX_BLACK=$MB \
  node tools/alphazero/selfplay/selfPlayWorker.mjs \
