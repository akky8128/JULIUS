...
export function makeRealDeps(cfg) {
  return {
    ...
    async train({ batchPath, round, net }) {
      ...
      const args = [cfg.python || "tools/nnue/train.py", "--data", batchPath, "--out-dir", cfg.outDir,
...
async function main() {
  ...
  const dir = get("--dir", "tools/alphazero/data/loop2");
  const seedNet = get("--seed-net", "models/genAZ001.json");
