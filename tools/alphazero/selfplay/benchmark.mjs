 ...
 *   gen137+ab          : evalWorker vsSearchNet --oppNet models/gen137.json --sdepth 32 --sms 120
 *   genAZ001+mcts      : evalWorker headToHead  --netB models/genAZ001.json
 *
 * 使い方（単体CLI）:
 *   node tools/alphazero/selfplay/benchmark.mjs \
 *     --net models/genAZ_r55.json --round 55 --dir tools/alphazero/data/loop2 --bench-games 40 --procs 10
...
function panel(full = false) {
  ...
  return [
    ...routine,
    { name: "gen137+ab", args: ["--mode", "vsSearchNet", "--oppNet", "models/gen137.json", "--sdepth", "32", "--sms", "120"], seedBase: 300000 },
    { name: "genAZ001+mcts", args: ["--mode", "headToHead", "--netB", "models/genAZ001.json"], seedBase: 400000 },
  ];
}
...
async function benchOne(opp, net, benchGames, procs, sims, round) {
  ...
  const tasks = shardGames.map((g, i) => () => run("node", [
    "tools/alphazero/selfplay/evalWorker.mjs",
...
async function main() {
  const dir = arg("--dir", "tools/alphazero/data/loop2");
