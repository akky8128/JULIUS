 ...
 *
 * js/ai/aiWorker.js（本番用・変更禁止）と同じメッセージ仕様を踏襲しつつ、
 ...
 * @param {string} jsonPath - 例 "models/gen012.json"（.bin は自動導出）
...
function loadNetForWorker(jsonPath) {
  ...
  // Worker 内の相対 fetch は Worker スクリプト(js/ai/)基準で解決されてしまうため、
  // ページ origin を基準に絶対URL化する（"/models/..." でも "models/..." でも動く）。
