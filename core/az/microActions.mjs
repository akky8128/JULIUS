 ...
 * 詳細仕様は research/alphazero/PLAN.md §3 を参照。
 *
 * ノード規約:
 ...
 *   80     ターン終了ノード
 *
 * 正解集合(a)のリファレンスは core/ai/moveGen.js の generateTurns/hashPosition。
 * 本モジュールは既存コード（js/ 以下）を一切変更せず、読み取り専用で利用する。
 */

import { hashPosition, MAX_CHAIN_LENGTH } from "../ai/moveGen.js";
import { normalizeBoard, maxSummonsFor, ruleMaxSummons, canEliminate, cloneBoard } from "../gameLogic.js";
...
// ブラウザ(Worker)には process が無いため、未定義時は既定レイアウト扱いにする。
const SEP_ELIM_ENV = typeof process !== "undefined" && process.env ? process.env.UL_SEP_ELIM : undefined;
export const SEPARATE_ELIMINATE = SEP_ELIM_ENV === "1" || SEP_ELIM_ENV === "true";
