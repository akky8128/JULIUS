// ===================================================================
// このファイルは js/ai/search.js のミラーです。
// クライアント側（js/）とサーバー側（functions/）で同じロジックを共有するため、
// 両ファイルは常に同期を保つ必要があります。
// js/ai/search.js を変更した場合は、このファイルにも同じ変更を反映してください。
// ===================================================================
...
export function findBestTurn(state, options = {}) {
  const {
    ...
    // ルート手選択の絶対マージン（評価値の絶対単位）。0（既定）は従来通り常に最善手を
    // 選ぶ決定論的な挙動。0 より大きい場合、最終的に完了した深さで「最善手から絶対値
    // sampleMargin 以内」のルート候補だけを対象に、スケール sampleMargin の softmax で
    // 確率的に選ぶ（対局の多様性を出すため）。評価値スプレッドに依存しないため、鋭い局面
    // でも最善手からの損失は最大でも sampleMargin に抑えられる。詰み/必勝手が見つかった
    // 場合はマージンを無視して必ずその手を選ぶ。
    sampleMargin = 0,
  } = options;
  ...
  let bestScore = -Infinity;
  let depthReached = 0;
  // 温度サンプリング用: タイムアウトなく完了した最後の深さにおける
  // 全ルート候補の { cand, score }（即勝ちで打ち切られた場合は null のまま）。
  let finalDepthScores = null;
  ...
  for (let depth = 1; depth <= maxDepth; depth++) {
    ...
    let alpha = -Infinity;
    const beta = Infinity;
    const depthScores = []; // 温度サンプリング用: この深さの { cand, score } 全件

    // 温度サンプリング時は、ルート候補の α-β 枝刈りを無効化してフルウィンドウで
    // 探索する。α を上げてしまうと2手目以降が狭いウィンドウで打ち切られ、
    // depthScores に記録されるのが「真の評価値」ではなく α 付近に圧縮された
    // 上界（fail-soft の打ち切り値）になり、劣る手が最善手と同点に見えてしまう。
    // すると絶対マージン内の band 判定に劣手が紛れ込み悪手を選んでしまうため、
    // sampleMargin>0 のときは各ルート手の正確なスコアを得る（探索は各子ノード内部の
    // α-β で引き続き枝刈りされるため、失われるのはルート兄弟間の枝刈りのみ）。
    const rootFullWindow = sampleMargin > 0;
    ...
    for (const cand of sortedRoot) {
      ...
      // rootFullWindow 時は -alpha を渡さず +Infinity(=-(-Infinity)) 相当のフルウィンドウで
      // 探索し、各ルート手の正確な評価値を得る。
      const childAlpha = rootFullWindow ? -Infinity : alpha;
      const score = -negamax(opp, childHash, depth - 1, -beta, -childAlpha, 1);
      undo();
      ...
      depthScores.push({ cand, score });

      if (score > depthBestScore) {
        depthBestScore = score;
        depthBestTurn  = cand;
        // rootFullWindow 時は alpha を上げない（兄弟の枝刈りを避け、全手を正確に評価する）。
        if (!rootFullWindow && score > alpha) alpha = score;
      }
    }
    ...
    // 温度サンプリング用スコア一覧は「タイムアウトなく全候補を評価し終えた深さ」
    // のみ採用する（タイムアウト時は一部候補が未評価のため不公平なサンプリングになる）。
    if (!timedOut) {
      finalDepthScores = depthScores;
    }
  ...
  // ── 絶対しきい値サンプリング ──
  // 必勝手（bestScore が WIN_SCORE 近傍）が見つかった場合はマージンを無視して確定手を維持する。
  // それ以外で sampleMargin>0 かつ複数候補があれば、「最善手の評価値 - sampleMargin」以上の
  // 手だけを band として抽出し、その中でスケール sampleMargin の softmax で確率的に選び直す。
  // 相対スプレッドに依存しないため、band 外（明確に劣る手）は決して選ばれず、選ばれる手の
  // 損失は最大でも sampleMargin に限定される（鋭い局面でも大損しない）。
  // rootFullWindow により finalDepthScores は真の評価値なので、band 判定は正確。
  const isForcedWin = bestScore >= WIN_SCORE - maxDepth * 2;
  if (sampleMargin > 0 && !isForcedWin && finalDepthScores && finalDepthScores.length > 1) {
    const maxS = Math.max(...finalDepthScores.map((x) => x.score));
    const band = finalDepthScores.filter((x) => x.score >= maxS - sampleMargin);
    if (band.length > 1) {
      const weights = band.map((x) => Math.exp((x.score - maxS) / sampleMargin));
      const total = weights.reduce((a, b) => a + b, 0);
      let r = rng() * total;
      let sampled = band[band.length - 1].cand;
      for (let i = 0; i < band.length; i++) {
        r -= weights[i];
        if (r <= 0) { sampled = band[i].cand; break; }
      }
      bestTurn = sampled;
    }
  }
